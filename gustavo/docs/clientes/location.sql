...
CREATE OR REPLACE FORCE EDITIONABLE VIEW "CA"."VW_CAD_CLIENTE_EBS_LOCATION" 
--("COUNTRY", "ADDRESS1", "ADDRESS2", "ADDRESS3", "ADDRESS4", "STATE", "CITY", "POSTAL_CODE", "ORIG_SYSTEM_REFERENCE", "ID_PESSOA") AS 
as
...
Union all
  select (lpad(pf.nr_cpf,9,0)||lpad(pf.dv_cpf,2,0)||'_' || CD_INSCRICAO_CRM  )  orig_system_reference --########Revisar informação CNPJ p_location_rec.orig_system_reference
        , p.id_pessoa id_pessoa
    ...
    from   
      ...
      inner join (SELECT max(pl.ID_PESSOA_LOGRADOURO) ID_PESSOA_LOGRADOURO, pl.id_pessoa from ca.cp_pessoa_logradouro pl group by pl.id_pessoa) log
       on log.id_pessoa = pes.id_pessoa     
      inner join (SELECT pl.ID_PESSOA_LOGRADOURO
                    , TRIM(nm_tipo_logradouro || ' ' || nm_logradouro) as ADDRESS1
                    , nr_endereco as ADDRESS2
                    , nm_bairro as ADDRESS3
                    , ds_complemento as ADDRESS4
